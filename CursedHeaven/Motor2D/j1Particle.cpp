...
void j1Particle::Draw()
{
	...
	App->render->BlitParticles(App->ps_manager->particle_tex, prop.pos.x + ((drawRect.w - prop.rectSize.w) / 2), prop.pos.y + ((drawRect.h - prop.rectSize.h) / 2), &prop.rect, &prop.rectSize, InperpolateColors(prop.startColor, prop.endColor, prop.fraction));
}
