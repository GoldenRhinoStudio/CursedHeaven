...
Squares::Squares(SCENE scene_to_remove, SCENE scene_to_appear, int transition, j1Color color, float time) : j1Transitions(time) {
	this->color = color;
...
void Squares::Start() {
	...
	if (transition == 1 || transition == 3) {
		int appear = (int)Interpolation(1, number*number, percentage);
		Draw(appear);
	}
...
void Squares::Exit() {
	...
	if (transition == 1 || transition == 3) {
		int appear = (int)Interpolation(number*number, 1, percentage);
		Draw(appear);
	}
...
void Squares::Draw(int appear, int width, int height) {
	...
	if (appear > number * number) appear = number * number;
