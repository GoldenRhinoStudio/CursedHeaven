...
bool j1Slime::Update(float dt, bool do_logic)
{
	...
	if (!dead) {
		...
		iPoint destination = { App->map->WorldToMap((int)App->entity->currentPlayer->collider->rect.x, (int)App->entity->currentPlayer->collider->rect.y + App->entity->currentPlayer->collider->rect.h) };
